...
2.Deleting task from {{FE Task(i)}}
    ...
    c)After deleting of item rebuild id from 1 to n(?optional)
...
function idFunc() {
    if (localStorage.getItem("tasks") == null) {
        ...
        let prevTask = JSON.parse(localStorage.getItem("tasks"))
        //Primary key for tasks based on previous task
        return prevTask[prevTask.length - 1].id + 1;
    }
}
